...
describe('Config', () => {
	describe('normalise', () => {
		...
		test('normalises the paths of a given config', () => {
			const result = Config.normalise(config, {})
		...
		test('normalises the paths with the config and options project path', () => {
			const result = Config.normalise(config, { project: './mock' })
